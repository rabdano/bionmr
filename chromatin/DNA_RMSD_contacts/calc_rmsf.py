...
# calculate rmsf
dna_bp_avg_com = np.mean(bp_coordinates, axis=1)
rmsf = np.zeros(n_dna_bp)
for i in range(n_dna_bp):
    cum_sf = 0
    for j in range(int(traj.size/stride)):
        sf = (np.linalg.norm(bp_coordinates[i, j, :] - dna_bp_avg_com[i, :]))**2
        assert np.sqrt(sf) < (a/2), ("sf =", sf, "- translation error (?)")
        cum_sf += sf
    rmsf[i] = np.sqrt(cum_sf / int(traj.size/stride))
