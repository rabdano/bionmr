...
for i, file in enumerate(basenames):
    ...
    for line in qdb_lines:
        atoms_cn.append(AtomNMR(line, emp_qdb='qdb'))
    ...
    # carbon and nitrogen
    for at in atoms_cn:
        if at.rId in residue_map:
            index = residue_map.index(at.rId)

            # combine contributions, replace random coil shift
            at.cs = at.REF  # reference chemical shift
            at.cs += at.bb_p  # preceding backbone effect
            at.cs += at.bb_s  # self backbone effect
            at.cs += at.bb_f  # following backbone effect
            at.cs += at.chi_p  # preceding chi effect
            at.cs += at.chi_s  # self chi effect
            at.cs += at.HB_D  # direct HB effect
            at.cs += at.HB_I  # indirect HB effect
            
            # put chemical shift to corresponding table
            if at.aName == 'N':
                N_shifts[i, index] = at.cs
            if at.aName == 'Ca':
                CA_shifts[i, index] = at.cs
            if at.aName == 'Cb':
                CB_shifts[i, index] = at.cs
            if at.aName == 'CO':
                C_shifts[i, index] = at.cs
    
    if i == 0:
        for rId in residue_map:
            for at in (atoms_all + atoms_cn):
                if at.rId == rId:
...
with open('shifts_report.txt', 'w') as f:
    f.write('First tail:\n')
    f.write(str1)
    f.write('Second tail:\n')
    f.write(str2)
